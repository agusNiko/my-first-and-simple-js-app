let pokemonRepository = (function () {
  ...
  function add(pokemon) {
    ...
    if (typeof pokemon === 'object') {
      if (properties[0] === 'name' && properties[1] === 'detailsUrl') {
        pokemonList.push(pokemon);
      } else {
        let stringifiedObject = JSON.stringify(pokemon);
        alert(
          `${stringifiedObject} wrong format, the pokemon can not be added`
        );
      }
    } else {
    ...
    return fetch(apiUrl)
      ...
      .then(function (json) {
        ...
        json.results.forEach(function (item) {
          //json is the API object in json format. result is the Key of the object whose values are the pokemons. this also creates an object forEach pokemon
          let pokemon = {
            name: item.name,
            detailsUrl: item.url,
          };
  ...
  function loadDetails(item) {
    ...
    return fetch(url)
      ...
      .then(function (details) {
        //details is the parameter that would be the return value of the previous function
  ...
  function showModal(title, text, type, imgUrl, nextPokemon, previousPokemon) {
    ...
    modalClose.addEventListener('click', function () {
      //calls the hideModal() so removes the .is-visible. it Has to be call from inside this function because here is the created element.
      hideModal();
    });
    //Modal title
    ...
    //Modal container for type and height

    let paragrafContainer = document.createElement('div');
    paragrafContainer.classList.add('paragraf-container')
    contentContainer.appendChild(paragrafContainer);
    ...
    paragrafContainer.appendChild(contentElement);

    //modal Paragraf 2;
    let typeElement = document.createElement('p');
    typeElement.innerText = type;
    paragrafContainer.appendChild(typeElement);
